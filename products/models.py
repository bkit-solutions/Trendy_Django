...
class Product(models.Model):
    ...
    def main_image(self):
        """
        Returns the primary image for the product, or the first available image if no primary is set.
        """
        primary_image = self.images.filter(is_primary=True).first()
        if primary_image:
            return primary_image.image
        # If no primary, return the first image if any exist
        first_image = self.images.first()
        if first_image:
            return first_image.image
        # Return a placeholder if no images exist
        return 'path/to/default/placeholder.png' # <<< IMPORTANT: Change this to a path to a default image if no product images exist
