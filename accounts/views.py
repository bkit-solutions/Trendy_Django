# accounts/views.py - FINAL CORRECTED VERSION
...
# Keep all rest_framework imports because ViewSets are defined in this file
...
# IMPORTANT: Ensure Product is imported here
from products.models import WishlistItem, Product 
...
# Recommendation Engine Imports (already correctly added by you)
from products.recommendations import RecommendationEngine 
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user is not None:
                ...
                if user.is_superuser:
                    return redirect('admin:index')
                return redirect('dashboard') # Assuming 'dashboard' is now correctly mapped
            else:
...
def dashboard(request):
    ...
    # --- Recommendation Engine Integration for Dashboard ---
    engine = RecommendationEngine()
    
    # THIS LINE WAS MISSING FROM YOUR PROVIDED CODE FOR THE dashboard VIEW
    # It defines 'dashboard_recommendations'
    dashboard_recommendations = engine.get_personalized_recommendations(user=request.user, limit=4)
    ...
    context = {
        ...
        'dashboard_recommendations': dashboard_recommendations, # This variable is now defined
...
def account_wishlist(request):
    ...
    # --- Recommendation Engine Integration for Wishlist ---
    engine = RecommendationEngine()

    # Get product IDs from the wishlist
    wishlist_product_ids = [item.product.id for item in wishlist_items]

    # Get recommendations based on the products in the wishlist
    # The engine will handle fallbacks if the wishlist is empty
    wishlist_based_recommendations = engine.get_similar_to_products(
        product_ids=wishlist_product_ids, 
        limit=8, 
        exclude_ids=wishlist_product_ids # Don't recommend items already wishlisted
    )

    return render(request, 'accounts/account_wishlist.html', {
        'wishlist_items': wishlist_items,
        'wishlist_based_recommendations': wishlist_based_recommendations, # Add this
        'active_tab': 'wishlist'
    })